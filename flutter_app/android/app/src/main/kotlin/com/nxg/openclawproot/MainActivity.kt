 ...
 override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
  ...
  MethodChannel(flutterEngine.dartExecutor.binaryMessenger, CHANNEL).setMethodCallHandler { call, result ->
   when (call.method) {
    ...
    "isPtyProcessAlive" -> {
     result.success(TerminalSessionService.isPtyProcessAlive(applicationContext))
    }
    "getPtyPid" -> {
     result.success(TerminalSessionService.getPtyPid(applicationContext))
    }
    "savePtyPid" -> {
     val pid = call.argument<Int>("pid") ?: -1
     if (pid > 0) {
      TerminalSessionService.savePtyPid(applicationContext, pid)
      result.success(true)
     } else {
      result.error("INVALID_ARGS", "pid must be > 0", null)
     }
    }
    "clearPtyPid" -> {
     TerminalSessionService.clearPtyPid(applicationContext)
     result.success(true)
    }
    "startNodeService" -> {
     try {
  ...
  if (requestCode == SCREEN_CAPTURE_REQUEST) {
   if (resultCode == Activity.RESULT_OK && data != null) {
    ...
    Thread {
     ...
     while (ScreenCaptureService.resultPath == null && System.currentTimeMillis() - startTime < timeout) {
      Thread.sleep(200)
     }
